...
class IrModelImpex(models.AbstractModel):
    ...
    def process(self):
        ...
        for record in self:
            if record.new_thread or record.test_mode:
                thread = Thread(
                    target=IrModelImpex._process_with_new_cursor, args=(self,)
                )
                thread.start()
    ...
    def _kill_impex(self, hours=0):
        ...
        if hours:
            limit_date = date_utils.subtract(fields.Datetime.now(), hours=hours)
            domain.append(("create_date", "<=", limit_date))
        impex_infos = self.search_read(domain, ["pid"], order="id")
        impex_ids = [
            impex["id"] for impex in impex_infos if not psutil.pid_exists(impex["pid"])
        ]
        if impex_ids:
