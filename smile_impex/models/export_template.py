...
class IrModelExportTemplate(models.Model):
    ...
    export_ids = fields.One2many(
        "ir.model.export", "export_tmpl_id", "Exports", readonly=True, copy=False
    )
    ...
    filter_type = fields.Selection(
        [("domain", "Domain"), ("method", "Method")], required=True, default="domain"
    )
    ...
    def _get_client_action(self):
        for template in self:
            template.client_action = bool(template.server_action_id.binding_model_id)

    @api.constrains("unique", "do_not_store_record_ids")
    def _check_export_template_config(self):
        if any(
            template.unique and template.do_not_store_record_ids for template in self
        ):
            raise UserError(
                _("Exported records storing is required " "if export must be unique")
            )
    ...
    def _get_res_ids(self, *args):
        ...
        if self.filter_type == "domain":
            domain = safe_eval(self.filter_domain or "[]", self._get_eval_context())
            ...
            if not (self.filter_method and hasattr(model_obj, self.filter_method)):
                raise UserError(
                    _("Can't find method: %s on object: %s")
        ...
        if self.unique:
            res_ids -= set(
                sum([safe_eval(export.record_ids) for export in self.export_ids], [])
            )
        return list(res_ids)
