...
class IrModelExport(models.Model):
    ...
    def _execute(self):
        ...
        if not self.record_ids:
            raise UserError(
                _(
                    "You cannot regenerate this export "
        ...
        if record_ids or self.export_tmpl_id.force_execute_action:
            records = (
                self.env[self.export_tmpl_id.model_id.model]
                .browse(record_ids)
                .with_context(export_tmpl_id=self.export_tmpl_id.id)
            )
            if self.export_tmpl_id.method:
                if self._context.get("original_cr") and not self._context.get(
                    "force_use_new_cursor"
                ):
                    new_env = self.env(
                        cr=self._context["original_cr"], context=records._context
                    )
